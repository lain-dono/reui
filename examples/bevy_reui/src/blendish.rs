use reui::{Color, Offset, Rect, Rounding, Stroke};

type Canvas<'a> = reui::Canvas<'a, bevy::asset::Handle<bevy::image::Image>>;
...
fn offset_color(color: Color, delta: i32) -> Color {
    if delta != 0 {
        ...
        Color {
            red: (color.red + offset).clamp(0.0, 1.0),
            green: (color.green + offset).clamp(0.0, 1.0),
            blue: (color.blue + offset).clamp(0.0, 1.0),
            alpha: color.alpha,
        }
    } else {
        color
