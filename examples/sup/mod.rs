...
pub fn draw_thumbnails(ctx: &mut Canvas, rr: Rect, images: &[Image], time: f32) {
    ...
    for (i, &image) in images.iter().enumerate() {
        ...
        ctx.draw_rrect(
            RRect::new([tx, ty].into(), [thumb, thumb].into(), 5.0),
            paint,
        );
    ...
    ctx.draw_rect(
        rect(x + 4.0, y + height - 6.0, width - 8.0, 6.0),
        Paint::gradient(Gradient::Linear {
            from: [x, y + height],
            to: [x, y + height - 6.0],
            inner_color: 0xFF_C8C8C8,
    ...
    let rrect = RRect::new(
        [
            x + width - 12.0 + 1.0,
            y + 4.0 + 1.0 + (height - 8.0 - scrollh) * u1,
        ]
        .into(),
        [8.0 - 2.0, scrollh - 2.0].into(),
        2.0,
    );
    let paint = Paint::box_gradient(
        rect(
            x + width - 12.0 - 1.0,
            y + 4.0 + (height - 8.0 - scrollh) * u1 - 1.0,
            8.0,
            scrollh,
        ),
        3.0,
