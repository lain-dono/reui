...
impl PictureRecorder {
    ...
    pub(crate) fn commands(&self) -> &[f32] {
        &self.commands
    }

    pub(crate) fn extend(&mut self, commands: &mut [f32], xform: Transform) {
        if commands[0] as u32 != CLOSE && commands[0] as u32 != WINDING {
            self.cmd.x = commands[commands.len() - 2];
            self.cmd.y = commands[commands.len() - 1];
        }
    ...
    pub fn move_to(&mut self, p: Offset, xform: Transform) {
        let [x, y] = xform.apply(p.into());
        self.commands.extend_from_slice(&[MOVETO as f32, x, y]);
    }

    pub fn line_to(&mut self, p: Offset, xform: Transform) {
        let [x, y] = xform.apply(p.into());
        self.commands.extend_from_slice(&[LINETO as f32, x, y]);
    }

    pub fn bezier_to(&mut self, p1: Offset, p2: Offset, p3: Offset, xform: Transform) {
        let [x1, y1] = xform.apply(p1.into());
        let [x2, y2] = xform.apply(p2.into());
        let [x3, y3] = xform.apply(p3.into());
        self.commands
            .extend_from_slice(&[BEZIERTO as f32, x1, y1, x2, y2, x3, y3]);
    }

    pub fn quad_to(&mut self, c: Offset, p1: Offset, xform: Transform) {
        const FIX: f32 = 2.0 / 3.0;
        let p0 = self.cmd;
        self.bezier_to(p0 + (c - p0) * FIX, p1 + (c - p1) * FIX, p1, xform);
    }

    pub fn rect(&mut self, r: Rect, xform: Transform) {
        self.extend(
            &mut [
                MOVETO as f32,
                r.min.x,
    ...
    pub fn ellipse(&mut self, cx: f32, cy: f32, rx: f32, ry: f32, xform: Transform) {
        self.extend(
            &mut [
                MOVETO as f32,
                cx - rx,
    ...
    pub fn circle(&mut self, cx: f32, cy: f32, r: f32, xform: Transform) {
        self.ellipse(cx, cy, r, r, xform);
    }

    pub fn arc(&mut self, c: Offset, r: f32, a0: f32, a1: f32, dir: Winding, xform: Transform) {
        ...
        self.extend(&mut vals[..nvals], xform);
    }

    pub fn arc_to(&mut self, p1: Offset, p2: Offset, radius: f32, dist_tol: f32, xform: Transform) {
        let p0 = self.cmd;
        let tol = dist_tol;
        ...
        self.arc(Offset::new(cx, cy), radius, a0, a1, dir, xform);
    }
}
