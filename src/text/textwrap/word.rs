...
pub(crate) fn skip_ansi_escape_sequence<I: Iterator<Item = char>>(ch: char, chars: &mut I) -> bool {
    ...
    ch == CSI.0 && chars.next() == Some(CSI.1) && chars.any(|ch| ANSI_FINAL_BYTE.contains(&ch))
}
...
///
/// ```
/// use reui::text::textwrap::word::display_width;
///
...
impl<'a> Word<'a> {
    ...
    ///
    /// ```
    /// use reui::text::textwrap::word::Word;
    /// assert_eq!(
