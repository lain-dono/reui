...
fn resolve_font(node: svgtree::Node, state: &State) -> Option<fontdb_ext::Font> {
    ...
    */
    ...
    let id = match state.opt.fontdb.query(&query) {
        Some(v) => v,
        None => {
            //log::warn!("No match for '{}' font-family.", font_family);
            return None;
        }
    };
...
pub fn resolve_rotate_list(text_node: TextNode) -> Vec<f32> {
...
fn resolve_baseline_shift(node: svgtree::Node, state: &State) -> f32 {
    let mut shift = 0.0;
